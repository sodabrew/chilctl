...
import (
	...
	baseunits "github.com/martinlindhe/unit"
)

// Temperature contains a temperature value in kelvin.
type Temperature = baseunits.Temperature
...
func FromCelsius(t float64) Temperature {
	return baseunits.FromCelsius(t)
}

// FromCelsius returns a temperature from a value in degrees celcius.
func FromFahrenheit(t float64) Temperature {
	return baseunits.FromFahrenheit(t)
}

// Current represents electric current.
type Current = baseunits.ElectricCurrent

const Ampere = baseunits.Ampere

// Voltage represents electric Voltage.
type Voltage = baseunits.Voltage

const Volt = baseunits.Volt

// Power represents energy over time.
type Power = baseunits.Power

// Power constants.
const (
	Watt     Power = baseunits.Watt
	Kilowatt Power = baseunits.Kilowatt
)

// PowerFromIV multiples current and voltage to get power.
func PowerFromIV(i Current, v Voltage) Power {
	return baseunits.Watt * baseunits.Power(i.Amperes()*v.Volts())
}
...
func (sh SpecificHeat) TimesMassDeltaTemp(m Mass, t Temperature) Energy {
	return baseunits.Kilojoule * baseunits.Energy(float64(sh)*m.Kilograms()*t.Kelvin())
}

// Mass is a floating point mass value.
type Mass = baseunits.Mass
...
type Energy = baseunits.Energy

// Volume is a floating point volume value.
type Volume = baseunits.Volume

// Volume values.
const (
	CubicMeter = baseunits.CubicMeter
	Liter      = baseunits.Liter
)
